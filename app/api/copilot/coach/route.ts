...
export async function POST(req: Request) {
  ...
  let lane: Lane, context: ContextPacket
  try {
    const body = object(await readRepoJson(req, 100_000), ['lane', 'context'])
    if (!['talk', 'guide', 'review'].includes(String(body.lane))) throw new Error('Invalid lane')
    lane = body.lane as Lane; context = parseContext(body.context)
  } catch (error) {
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      try {
        ...
        const request = { model, system: coachPrompt(lane), evidence: JSON.stringify(context), signal, maxTokens: lane === 'talk' ? 512 : lane === 'review' ? 2200 : 3400 }
        if (lane === 'talk') {

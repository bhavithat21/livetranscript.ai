// One-time, guarded integration. Only allowlisted source files are changed.
// The resulting diff stays on the feature branch for review and full CI.
...
if (!content.includes('mod repo_capture;')) {
  ...
  if (blob !== 'c3e798fa3f2c1cdccbd87dda9a0dc2502721b4d4') throw new Error('Native shell changed; rebase registration manually')
  function replace(before, after) { if (content.split(before).length !== 2) throw new Error('Unexpected native registration anchor'); content = content.replace(before, after) }
...
if (!ui.includes("import { NativeCaptureControls }")) {
  function replace(before, after) { if (ui.split(before).length !== 2) throw new Error('Unexpected UI integration anchor'); ui = ui.replace(before, after) }
...
console.log('Registered read-only native capture, explicit UI selection, and emergency/exit teardown')

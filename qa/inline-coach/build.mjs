...
await build({ configFile: false, root: path('.'), plugins: [react()], resolve: { alias: {
  '@': path('../..'), '@tauri-apps/api/core': path('./bridge.ts'), '@tauri-apps/api/event': path('./bridge.ts'),
}}, build: { outDir: path('../../qa-results/inline-coach/bundle'), emptyOutDir: true, rollupOptions: { output: { inlineDynamicImports: true } } } })

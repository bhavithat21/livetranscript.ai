...
function CoachWorkspace({ controller, screen, getQuestionTranscript = EMPTY_TRANSCRIPT, getConversation, permission, objective: presetObjective, onActivity }: Omit<RepositoryCoachProps, 'onReady' | 'transport' | 'captureTransport'> & Resources) {
  ...
  return <section className={styles.root} aria-label="Repository coach" data-testid="repository-coach">
    <InlineCoach observer={screen} state={state} capture={capture} active={inline} onExit={closeInline} />
    ...
    {state.status !== 'idle' && <>
      ...
      {native && <details className={`${styles.details} ${styles.main}`}><summary>Inline Coach · changes beside code (experimental)</summary>
        ...
        <button className={styles.button} disabled={!running || selecting} onClick={() => void inlineTargets().then(items => { setTargets(items); setTargetId(String(items[0]?.id ?? '')) }).catch(() => setError('Update the desktop installer and allow Screen Recording to select an IDE window.'))}>Find IDE windows</button><button className={styles.button} onClick={() => saveFile(`visual-tracking-${state.sessionId}.json`, JSON.stringify(screen.trackingReport(), null, 2))}>Export tracking metrics</button>

  ...
  return <div className="space-y-5 sm:pr-[var(--mock-panel-w,0px)]" style={running && visible ? { '--mock-panel-w': `${panel.width}px` } as CSSProperties : undefined}>
    <section className="space-y-6 rounded-xl border border-black/[0.07] bg-white/50 p-5 sm:p-7">
      ...
      <div className="flex flex-wrap gap-2">
        {!running ? <button className="btn-signal" disabled={blocked || !scenario.trim()} onClick={begin}>Run live system<span className="sr-only">Open live copilot for mock test</span></button> : <>
          ...
          <button className="btn-signal" onClick={() => void finish()}>Finish &amp; review</button>
        </>}

...
export function InterviewFeedback({ sessions, selectedId, onSelect, store }: {
  ...
  return <div className="space-y-5">
    <section className="reader-surface rounded-2xl p-5 sm:p-6">
      ...
      <p className="mt-2 text-sm text-black/60">Live sessions review captured candidate answers. Mock tuning sessions review the copilot system: output quality, observed failures, request latency and proposed live-instruction changes.</p>
      {!sessions.length ? <div className="mt-5 rounded-xl border border-dashed border-black/20 p-6 text-sm">No completed sessions yet. Finish live capture or a mock system test, or paste an existing transcript below.</div> : <>
        <label className="mt-5 block space-y-2 text-sm">Session<select className="w-full rounded-xl border border-black/15 bg-transparent p-3" value={session?.id ?? ''} onChange={(e) => { onSelect(e.target.value); setConfirmDelete(null); setError(null) }}>{sessions.map((item) => <option key={item.id} value={item.id}>{item.kind === 'tuning' ? 'System tuning' : item.kind === 'mock' ? 'Legacy practice' : 'Live'} · {item.title} · {new Date(item.createdAt).toLocaleString()}</option>)}</select></label>
        {session && <>
          <p className="mt-3 text-xs leading-relaxed text-black/60">{session.captureNote}</p>
          {session.kind === 'tuning' && <p className="mt-3 rounded-xl bg-black/5 p-3 text-sm">Evaluating the AI system, not the candidate. Suggested instruction changes remain recommendations until tested and applied in Mock.</p>}
          <div className="mt-4 flex flex-wrap gap-2"><button className="btn-signal" disabled={!!reviewingId || !session.transcript.trim()} onClick={() => void review()}>{reviewingId === session.id ? 'Reviewing session…' : session.feedback ? 'Regenerate feedback' : 'Generate feedback'}</button>{reviewingId && <button className="btn-ghost" onClick={cancel}>Cancel review</button>}<button className="btn-ghost" onClick={() => exportSession(session)}>Export report &amp; transcript</button><button className="btn-ghost" disabled={reviewingId === session.id} onClick={() => setConfirmDelete(session.id)}>Delete session</button></div>
          ...
          {!session.feedback && <p className="mt-4 text-sm text-black/50">Choose Generate feedback to send this transcript or system test report to the configured AI provider. No report has been generated yet.</p>}
          <details className="mt-5 rounded-xl border border-black/10 p-4"><summary className="cursor-pointer text-sm font-medium">View captured transcript / system tests</summary><pre className="mt-4 max-h-[50vh] overflow-y-auto whitespace-pre-wrap break-words font-sans text-sm leading-relaxed">{session.transcript}</pre></details>
        </>}
      </>}

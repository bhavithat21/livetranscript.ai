...
function ProfileStatus() {
  ...
  return <div className="mb-4 text-xs text-black/60"><p>Live calibration revision {state.active.revision} · Tune in Mock, then apply to Live on this browser.</p>{error && <p role="alert">{error}</p>}</div>
}
export function InterviewWorkspace({ ownerId }: { ownerId: string }) {
  ...
  return <InterviewTuningProvider ownerId={ownerId}><main className="mx-auto min-h-dvh max-w-6xl px-4 pb-16 pt-5 text-ink sm:px-6">
    <header className="mb-6 flex flex-wrap items-center justify-between gap-4"><div onClickCapture={guardNavigation}><HomeMenu /></div><div className="min-w-0"><h1 className="font-[family-name:var(--font-serif)] text-3xl sm:text-4xl">Interview workspace</h1><p className="mt-1 text-sm text-black/60">Simulate. Tune. Go live. Review.</p></div></header>
    <div role="tablist" aria-label="Interview modes" className="glass mb-5 flex gap-1 overflow-x-auto rounded-2xl p-1.5">{TABS.map((item, index) => <button key={item.id} id={`interview-tab-${item.id}`} role="tab" aria-selected={tab === item.id} aria-controls={`interview-panel-${item.id}`} tabIndex={tab === item.id ? 0 : -1} onKeyDown={(event) => tabKey(event, index)} onClick={() => setTab(item.id)} className={`min-h-11 shrink-0 flex-1 rounded-xl px-4 py-3 text-sm font-medium transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[color:var(--signal)] ${tab === item.id ? 'bg-[color:var(--signal)] text-white' : 'text-black/60 hover:bg-black/5'}`}>{item.label}</button>)}</div>
    <ProfileStatus />
    {active && ((liveActive && tab !== 'live') || (mockActive && tab !== 'mock')) && <div role="status" className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-black/15 p-4 text-sm"><span>{liveActive ? 'Live interview remains active. Switching tabs does not stop audio capture.' : 'Your mock system test is in progress; the copilot and tuning draft are preserved.'}</span><button className="btn-ghost" onClick={() => setTab(liveActive ? 'live' : 'mock')}>Return to active interview</button></div>}
    ...
    <footer className="mt-6 text-xs leading-relaxed text-black/60">The latest 20 completed sessions and reports are saved in this browser for your signed-in account, not synced to other devices. Export important sessions before clearing browser data or exceeding that limit. Calibration profiles are also account/device-local and apply to this Interview workspace. Anyone with access to this browser profile may read local storage. AI requests send the selected scenario, transcript and grounding to the configured providers. No model weights are trained automatically.</footer>
  </main></InterviewTuningProvider>
}

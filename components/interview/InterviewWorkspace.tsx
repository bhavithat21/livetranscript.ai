...
function currentView(): InterviewView { const value = window.location.hash.slice(1); return value === 'mock' || value === 'feedback' ? value : 'live' }
function subscribeView(listener: () => void) { window.addEventListener('hashchange', listener); window.addEventListener('popstate', listener); return () => { window.removeEventListener('hashchange', listener); window.removeEventListener('popstate', listener) } }
function setView(view: InterviewView) { if (window.location.hash === `#${view}`) return; window.history.pushState(window.history.state, '', `#${view}`); window.dispatchEvent(new HashChangeEvent('hashchange')) }
function ProfileStatus() {
  const { state, error } = useInterviewTuning()
  return <div className={styles.profileStatus}><span><Check size={13} aria-hidden />Live profile <strong>v{state.active.revision}</strong></span>{error && <p role="alert" className={styles.error}>{error}</p>}</div>
}
function Workspace({ ownerId }: { ownerId: string }) {
  const view = useSyncExternalStore(subscribeView, currentView, () => 'live' as const)
  const [liveActive, setLiveActive] = useState(false), [mockActive, setMockActive] = useState(false)
  const [selectedId, setSelectedId] = useState<string | null>(null), [navigationError, setNavigationError] = useState<string | null>(null)
  ...
  useEffect(() => { if (!active) return; const beforeUnload = (event: BeforeUnloadEvent) => { event.preventDefault(); event.returnValue = '' }; window.addEventListener('beforeunload', beforeUnload); return () => window.removeEventListener('beforeunload', beforeUnload) }, [active])
  const completed = useCallback((session: InterviewSession) => { store.add(session); setSelectedId(session.id); setView('feedback'); setNavigationError(null) }, [store])
  function guardNavigation(event: MouseEvent<HTMLAnchorElement>) { if (!active) return; event.preventDefault(); setNavigationError('Finish the active interview before leaving this workspace. Switching between Interview views is safe.') }
  ...
  return <WorkspaceShell active="interview" interviewView={view} onInterviewViewChange={setView} onNavigate={guardNavigation}>
    <main className={styles.workspace}>
      <header className={styles.pageHeader}><div><p className={styles.eyebrow}>Your interview workspace</p><h1>{current.label}</h1><p className={styles.description}>{current.description}</p></div><div className={styles.headerMeta}><ProfileStatus />{active && <span className={styles.activeLabel}><Activity size={13} aria-hidden />Session active</span>}</div></header>
      {!active && <nav aria-label="Repository workflows" style={{ display: 'flex', flexWrap: 'wrap', gap: '12px 20px', marginBottom: 20, fontSize: 13 }}><Link href="/interview/repository" onClick={guardNavigation}>Open repository guidance</Link><Link href="/interview/replay" onClick={guardNavigation}>Repository Replay Lab</Link><Link href="/interview/benchmarks" onClick={guardNavigation}>Measure live models</Link></nav>}
...
export function InterviewWorkspace({ ownerId }: { ownerId: string }) { return <InterviewTuningProvider ownerId={ownerId}><Workspace ownerId={ownerId} /></InterviewTuningProvider> }

...
import { coverage, exportReplay, importReplay, safePath } from '@/lib/repo/live/engine'
...
export function RepositoryWorkspace({ replay = false }: { replay?: boolean }) {
  ...
  const [running, setRunning] = useState(false), [consent, setConsent] = useState(false), [source, setSource] = useState<'both' | 'system' | 'mic'>('system'), [error, setError] = useState(''), [note, setNote] = useState(''), [exportConsent, setExportConsent] = useState(false)
  const [cursor, setCursor] = useState(0), [playing, setPlaying] = useState(false), [importedReplay, setImportedReplay] = useState(false), [railOpen, setRailOpen] = useState(false)
  ...
  const questionText = useCallback(() => detectionTranscript((source === 'mic' ? getMic() : getCall()).filter(v => v.capturedAt >= beginAt.current), source === 'both' ? getMic().filter(v => v.capturedAt >= beginAt.current) : []), [getCall, getMic, source])
  ...
  const requestStop = useCallback(() => { life.current++; setRunning(false); observer.stop(); void call.stop(); void mic.stop() }, [observer.stop, call.stop, mic.stop])
  ...
  useEffect(() => {
    if (!replay || importedReplay) return
    replaceReplay(replayPrefix(fixture.events, cursor))
  }, [replay, importedReplay, cursor, fixture.events, replaceReplay])
  useEffect(() => {
    if (!playing || importedReplay) return
    const timer = setInterval(() => setCursor(value => Math.min(value + 1, fixture.events.length)), 900)
    return () => clearInterval(timer)
  }, [playing, importedReplay, fixture.events.length])
  async function start() {
    ...
    try {
      ...
      if (token === life.current && source !== 'system') await mic.start('mic', keyterms)
  ...
  async function importFolder(files: FileList | null) {
    ...
    try {
      for (const file of Array.from(files).slice(0, 120)) {
        ...
        if (!safePath(path) || !/\.(?:ts|tsx|js|jsx|mjs|py|java|cs|go|rs|json|md|yml|yaml|toml|xml|sql)$/.test(path) || /(^|\/)(?:node_modules|\.git|dist|build|vendor)\//.test(path)) continue
        bytes += file.size
        if (file.size > 150000 || bytes > 600000) throw new Error('Folder import exceeds the 600 KB source budget. Select a smaller relevant folder.')
  ...
  async function loadReplay(file?: File) {
    if (!file) return
    try { if (file.size > 8000000) throw new Error('Replay must be under 8 MB.'); const parsed = importReplay(await file.text()); requestStop(); setPlaying(false); setImportedReplay(true); replaceReplay(parsed); setError('') }
  ...
  function download() {
    ...
    try { const text = exportReplay(state); const url = URL.createObjectURL(new Blob([text], { type: 'application/json' })); const a = document.createElement('a'); a.href = url; a.download = 'repository-session-replay.json'; a.click(); setTimeout(() => URL.revokeObjectURL(url), 1000) }
    catch (e) { setError(e instanceof Error ? e.message : 'Export failed') }
  }
  const next = state.navigation ?? nextObservation(state), compiled = compileContext(state), summaries = coverage(state)
  const hasSession = running || state.seq > 0 || replay || importedReplay
  const isReplay = replay || importedReplay
  ...
  function guardNavigation(event: React.MouseEvent<HTMLAnchorElement>) { if (running) { event.preventDefault(); setError('End the repository session before leaving. Your captured evidence remains available until you leave this page.') } }
  return <WorkspaceShell active="repository" onNavigate={guardNavigation}>
    <main className={styles.page}>
      ...
      {!hasSession && <section className={styles.setup}>
        ...
        <div className={styles.field}><span>Audio source</span><select aria-label="Repository audio source" className={styles.input} value={source} onChange={e => setSource(e.target.value as typeof source)}><option value="system">Interviewer / system audio</option><option value="both">System audio + my microphone</option><option value="mic">Microphone (mock interviewer)</option></select></div>
        ...
        <p className={styles.subtitle}>Audio and screen sharing are separate, explicit choices. This app does not control the assessment IDE, execute model-suggested commands, or bypass platform restrictions.</p>
      </section>}
      {hasSession && <>
        ...
        <input className={styles.hidden} ref={fileInput} type="file" accept="image/png,image/jpeg,image/webp" aria-label="Upload repository screenshot" onChange={e => { const file = e.target.files?.[0]; if (file) void observer.upload(file); e.target.value = '' }} />
        ...
        <div className={styles.grid}>
          <div className={styles.main} data-testid="repo-answer-column">
            ...
            <section className={styles.card} aria-label="Code checks"><h2>Check</h2>{state.plan?.checks.length ? <ul className={styles.list}>{state.plan.checks.map((check, i) => <li key={i}><strong>{check.severity}: </strong>{check.text}</li>)}</ul> : <p className={styles.muted}>Reviews distinguish suspected errors from equivalent implementations. A text match alone does not prove correctness.</p>}{state.plan?.missingEvidence.length ? <details className={styles.details} open><summary>Evidence still needed</summary><ul className={styles.list}>{state.plan.missingEvidence.map((v, i) => <li key={i}>{v}</li>)}</ul></details> : null}</section>
            <section className={styles.card} aria-label="Verification"><h2>Verify</h2><p className={styles.muted}>Run commands in your permitted IDE. Clear old terminal output first. “Mark test started” records the current code revision; it does not execute anything.</p>{(state.plan?.verify ?? ['Run the relevant existing test in your IDE.']).map((command, i) => <div key={i} className={styles.edit}><pre className={styles.code}><code>{command}</code></pre>{!isReplay && running && <button className={styles.button} onClick={() => engine.dispatch({ kind: 'test-start', data: { command } })}>Mark test started</button>}</div>)}{state.testRuns.map(run => <p key={run.id} className={styles.body}><span className={styles.tag}>{run.status}</span> Code r{run.codeRevision}{run.passed !== null && ` · ${run.passed} passed`}{run.failed !== null && ` · ${run.failed} failed`} · screen-observed, not independently executed.</p>)}</section>
            {(isReplay || !running) && <section className={styles.card}><h2>Feedback & replay</h2><textarea aria-label="Repository feedback" className={styles.input} maxLength={2000} rows={3} placeholder="What was wrong, missing, or useful?" value={note} onChange={e => setNote(e.target.value)} /><div className={styles.controls}><button className={styles.button} onClick={() => verdict('pass')}>Pass</button><button className={styles.button} onClick={() => verdict('needs-work')}>Needs work</button></div><p className={styles.muted}>{state.feedback.length} saved feedback notes in this in-memory session.</p><div className={styles.feedback}><label className={styles.muted}><input type="checkbox" checked={exportConsent} onChange={e => setExportConsent(e.target.checked)} /> I reviewed this export. It contains source text and conversation events; remove confidential material before sharing.</label><div className={styles.controls}><button className={styles.button} disabled={!exportConsent} onClick={download}><Download size={14} />Export replay</button><button className={styles.button} onClick={() => replayInput.current?.click()}><Upload size={14} />Import replay</button></div></div></section>}
          </div>
          <aside className={styles.rail} aria-label="Repository evidence">
            <section className={styles.card}><h2>Look at</h2>{next ? <><p className={styles.path}>{next.path}{next.line !== null ? ` · line ${next.line}` : ''}</p>{next.symbol && <p className={styles.body}>{next.symbol}</p>}<p className={styles.muted}>{next.reason}</p>{state.navigation && <span className={styles.tag}>{state.navigation.status === 'observed' ? 'Target observed' : 'Waiting for the requested view'}</span>}</> : <p className={styles.muted}>{summaries.length ? 'The available file evidence is ready for analysis.' : 'Show the task and repository tree, then open the relevant entry point.'}</p>}</section>
            <details className={`${styles.card} ${styles.railDetails}`} open={railOpen} onToggle={e => setRailOpen(e.currentTarget.open)}><summary>Files and context ({summaries.length})</summary><h2>Observed repository</h2>{summaries.map(file => <div key={file.path} className={styles.file}><p className={styles.path}><FileCode2 size={13} style={{ display: 'inline', marginRight: 6 }} />{file.path}</p><p className={styles.muted}>{file.observedLines} lines · {file.complete ? 'visible through EOF' : 'partial'}{file.hasConflicts ? ' · prior anchors retired' : ''}{file.uncertainLines ? ` · ${file.uncertainLines} uncertain` : ''}</p></div>)}<div className={styles.metric}><span>{compiled.characters.toLocaleString()} context characters</span><span>~{compiled.estimatedTokens.toLocaleString()} tokens (estimate)</span><span>{engine.requests}/120 model requests</span><span>{state.discardedResults} stale results discarded</span></div></details>
      ...
      <footer className={styles.footer}><ShieldCheck size={13} style={{ display: 'inline', marginRight: 6 }} />Private in-memory workspace. Raw screen frames are not stored. Selected source, screenshots and audio are sent only to configured services when you enable them. End stops capture and inference. No hidden IDE access, auto-submission, or test bypass.</footer>
    </main>
  </WorkspaceShell>
}

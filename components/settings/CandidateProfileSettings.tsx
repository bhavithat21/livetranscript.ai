    ...
    <section aria-labelledby="settings-profile-heading">
      <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm leading-relaxed text-black/55">Give answers useful context from your actual experience and the role you want.</p>
        </div>
      ...
      <div className="space-y-4">
        {(Object.keys(PROFILE_FIELDS) as ProfileField[]).map((field) => {
          ...
          return (
            <div key={field} className="rounded-xl border border-black/10 bg-white/60 p-4 sm:p-5">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <label htmlFor={`settings-profile-${field}`} className="flex items-center gap-2 text-sm font-semibold"><FileText size={16} aria-hidden className="text-black/45" />{info.label}</label>
                <span className="text-xs tabular-nums text-black/45">{profile[field].length.toLocaleString('en-US')} / {info.limit.toLocaleString('en-US')}</span>
              </div>
              <p id={`settings-profile-${field}-help`} className="mt-2 text-xs leading-relaxed text-black/55">
              ...
              <textarea
                ...
                className="mt-3 block w-full resize-none rounded-lg border border-black/15 bg-white/70 px-3 py-2.5 text-sm leading-relaxed text-ink placeholder:text-black/35 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[color:var(--signal)]"
              />
              <div className="mt-3">
                <label htmlFor={`settings-profile-${field}-file`} className="text-xs font-medium text-black/60">{field === 'resume' ? 'Import resume text' : 'Import job description text'}</label>
                <input
                  ...
                  className="mt-1.5 block min-h-11 w-full min-w-0 cursor-pointer rounded-lg border border-black/10 px-3 py-2 text-xs text-black/60 file:mr-3 file:cursor-pointer file:rounded-md file:border-0 file:bg-black/5 file:px-3 file:py-1 file:text-xs file:font-medium file:text-ink"
                />
                <p id={`settings-profile-${field}-file-help`} className="mt-1.5 text-xs text-black/45">Plain text or Markdown, up to 64 KB. Import replaces this field.</p>
                {error && <p id={`settings-profile-${field}-error`} role="alert" className="mt-2 text-sm text-[color:var(--stop)]">{error}</p>}
                {loading === field && <div className="mt-2 flex items-center gap-3 text-xs text-black/55"><span role="status">Reading file…</span><button type="button" className="min-h-11 cursor-pointer font-medium underline underline-offset-4" onClick={() => { uploadVersion.current++; setLoading(null); if (inputs.current[field]) inputs.current[field]!.value = '' }}>Cancel import</button></div>}
              </div>
            </div>
          )
        })}
      ...
      <div className="mt-4 flex min-h-6 flex-wrap items-center gap-3">
        <p role={profile.savedNote ? 'alert' : 'status'} className={`text-xs leading-relaxed ${profile.savedNote ? 'text-[color:var(--stop)]' : 'text-black/55'}`}>
      ...
      <p className="mt-1 text-xs leading-relaxed text-black/45">Your background is included as context when you request AI answers. It is shared by the copilot and interview workflows on this device.</p>
    </section>
  )
}

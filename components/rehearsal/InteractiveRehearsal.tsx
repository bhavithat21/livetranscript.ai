...
function OwnedRehearsal({ ownerId }: { ownerId: string }) {
  ...
  function removeSaved(sessionId:string) {
    if(!window.confirm('Delete this saved session from this browser? This cannot be undone.'))return
    // Drain earlier writes before deletion and stop autosaving this ended run.
    // Otherwise an in-flight save could silently recreate the deleted record.
    if(coach.current?.sessionId===sessionId){flags.current.saveEnabled=false;dirty.current=false;setSaveEnabled(false);setSaveStatus('Local saving disabled for the deleted session. Export is still available.')}
    saveQueue.current=saveQueue.current.then(()=>deleteRehearsal(ownerId,sessionId)).then(()=>{
      if(mounted.current){setViewArchive(null);return refreshArchive()}
    }).catch(e=>{if(mounted.current)setStorageError(e instanceof Error?e.message:'Could not delete saved session.')})
  }
  ...
  return <main className="mx-auto max-w-[1600px] space-y-5 p-4 sm:p-6">
    ...
    <fieldset disabled={active} className="space-y-2 rounded-xl border p-4"><legend>Benchmark setup</legend>
      ...
      <label className="block">Run type<select aria-label="Run type" value={metadata.kind} onChange={e=>setMetadata({...metadata,kind:e.target.value as BenchmarkMetadata['kind']})}><option value="video-replay">Continuous video replay</option><option value="interactive">Interactive repository rehearsal</option></select></label>
      ...
      <label className="mr-4 inline-block">Split<select aria-label="Split" value={metadata.split} onChange={e=>setMetadata({...metadata,split:e.target.value as BenchmarkMetadata['split']})}><option value="tuning">Tuning</option><option value="holdout">Holdout — do not tune on this run</option></select></label>
      <label className="mr-4 inline-block">Playback speed<select aria-label="Playback speed" value={metadata.playbackRate} onChange={e=>setMetadata({...metadata,playbackRate:Number(e.target.value)})}>{[.5,1,1.25,1.5,2].map(r=><option key={r} value={r}>{r}×</option>)}</select></label>

...
export type InterviewConfig = { role: string; level: 'junior' | 'mid' | 'senior' | 'staff'; round: InterviewRound; questionCount: number; context: string }
...
function validTurns(value: unknown): value is InterviewTurn[] {
  return Array.isArray(value) && value.length <= 20 && value.every((turn) => record(turn) && typeof turn.question === 'string' && turn.question.length <= 6_000 && typeof turn.answer === 'string' && turn.answer.length <= 12_000)
}
export function isSession(value: unknown): value is InterviewSession {
  ...
  return typeof value.id === 'string' && value.id.length > 0 && value.id.length <= 200 &&
    (value.kind === 'live' || value.kind === 'mock' || value.kind === 'tuning') &&
...
export function historyKey(owner: string): string { return `lt.interviews.v1.${encodeURIComponent(owner)}` }
export function decodeHistory(raw: string | null, owner: string): InterviewSession[] {
  ...
  if (!record(data) || data.version !== 1 || data.owner !== owner || !Array.isArray(data.sessions)) throw new Error('Interview history could not be read for this account.')
...
// Kept for backwards compatibility with older candidate-practice histories/API.
export function mockTranscript(turns: InterviewTurn[]): string { return turns.map((turn, i) => `Question ${i + 1} — Interviewer: ${turn.question}\nCandidate: ${turn.answer}`).join('\n\n') }
export function reviewExcerpt(transcript: string): { transcript: string; coverage: string } {
  if (transcript.length <= MAX_REVIEW_CHARS) return { transcript, coverage: 'All captured transcript text was submitted for review. This does not establish that every participant was recorded.' }
  const marker = '\n\n[Middle of transcript omitted — do not infer what was said.]\n\n'
  const half = Math.floor((MAX_REVIEW_CHARS - marker.length) / 2)
  return { transcript: transcript.slice(0, half) + marker + transcript.slice(-half), coverage: `Partial review: only the beginning and end of a ${transcript.length.toLocaleString('en-US')}-character transcript were submitted (up to ${MAX_REVIEW_CHARS.toLocaleString('en-US')} characters). The middle was omitted.` }
}
...
export function interviewPrompt(request: InterviewRequest): { system: string; user: string } {
  if (request.action === 'question') return {
...
## System behavior
Summarize the questions, selected modes, calibration settings and responses actually observed.
## Failures and evidence
Compare each AI response with its held-out expected behavior if supplied. Cite the test number and short excerpts. Identify factual-grounding gaps, missed requirements, unclear reasoning, formatting issues and request errors. A human Pass is an annotation, not proof. Without code execution or authoritative evidence, mark correctness as unverified instead of claiming tests passed.
## Latency and coverage
Report only supplied measured request-to-first-text and total times. These exclude ASR and pre-request orchestration. Do not invent end-to-end latency, provider identities, scores, or statistical improvements. A single run is not a benchmark; different transcripts/history/context/images confound comparisons. Explicitly acknowledge excerpts and missing frames/retrieval context.
## Recommended live configuration changes
Propose concrete, bounded wording changes to the live response instructions, tied to observed failures. Suggestions do NOT apply automatically. Do not claim model weights were trained.
## Next regression scenarios
Give three test cases and explicit acceptance criteria to check before applying a profile. Never evaluate employability or predict a hiring outcome. Keep the report under 1200 words.`, user,
  }
  return {
...
Keep the report under 1200 words.`, user,
  }
}

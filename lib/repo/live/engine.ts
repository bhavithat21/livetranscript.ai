...
import type { CodePlan, EditSuggestion, EventData, RepoEvent, RepoSession, Stamp } from './types'

export const MAX_EVENTS = 600
export const MAX_REPLAY_BYTES = 8_000_000
const MAX_TEXT = 6000
const safeText = (v: unknown, max = MAX_TEXT): v is string => typeof v === 'string' && v.length <= max && !/[\u0000-\u0008\u000b\u000c\u000e-\u001f]/.test(v)
export function safePath(value: unknown): value is string {
  return typeof value === 'string' && value.length > 0 && value.length <= 320 && !/[\\:\r\n\t]/.test(value) && value.split('/').every(p => !!p && !['.', '..', '__proto__', 'constructor', 'prototype'].includes(p)) && !/(^|\/)(?:\.env(?:\.|$)|id_(?:rsa|ed25519)|credentials(?:\.|$)|secrets?(?:\.|$)|[^/]*\.(?:pem|key|p12|pfx)$)/i.test(value)
}
function obj(v: unknown): Record<string, unknown> {
  if (!v || typeof v !== 'object' || Array.isArray(v)) throw new Error('Expected an object')
  if (Object.keys(v).some(k => ['__proto__', 'constructor', 'prototype'].includes(k))) throw new Error('Invalid object key')
  return v as Record<string, unknown>
}
function txt(v: unknown, max = MAX_TEXT): string { if (!safeText(v, max)) throw new Error('Text exceeds its limit'); return v }
...
function arr(v: unknown, max: number): unknown[] { if (!Array.isArray(v) || v.length > max) throw new Error('Too many items'); return v }
...
export function observedLines(file: ScreenFile): Map<number, { text: string; confidence: number; captures: number[] }> {
  ...
  for (const part of file.fragments) {
    ...
    part.lines.forEach((text, index) => {
      const n = part.startLine! + index, old = lines.get(n)
      lines.set(n, { text, confidence: part.confidence, captures: old?.text === text ? [...new Set([...old.captures, part.capture])].slice(-6) : [part.capture] })
    })
  }
  return lines
}
...
/** Locates an exact, unique, confidently observed preimage; never manufactures line numbers. */
export function anchorEdit(file: ScreenFile | undefined, before: string): number | null {
  ...
  for (const block of sourceBlocks(file).filter(b => b.certain)) {
    ...
    while (start <= block.text.length) {
      const i = block.text.indexOf(before, start)
      if (i < 0) break
      hits.push(block.start + block.text.slice(0, i).split('\n').length - 1)
      start = i + Math.max(1, before.length)
    }
  }
...
/** Replay data is never evaluated as code, never executes commands, and never starts model calls. */
export function parseEvent(raw: unknown): RepoEvent {
  ...
  switch (e.kind) {
    case 'observation': {
      if (!['screen', 'file-import', 'fixture'].includes(String(d.origin))) throw new Error('Invalid observation origin')
      const observation = parseScreenObservation(d.observation)
      if (d.activePath !== null && (!safePath(d.activePath) || !observation.files.some(f => f.path === d.activePath))) throw new Error('Active file must be in this observation')
    ...
    default: throw new Error('Unknown repository event')
  }
}
function applyIntent(s: RepoSession, utterance: string): RepoSession {
  ...
  if (/\b(?:prioritize|optim[iz]+e for)\s+readability/i.test(utterance)) additions.push('Prioritize readability over refactoring.')
  ...
  if (additions.length) next = { ...next, constraints: [...new Set([...next.constraints, ...additions])].slice(-30) }
  return next
}
export function parseTestSummary(text: string): { passed: number | null; failed: number | null; status: 'pass' | 'fail' } | null {
  // Only explicit terminal summaries, not code comments or a model's assertion.
  const clean = text.replace(/\u001b\[[0-9;]*m/g, '')
  const relevant = clean.split('\n').filter(l => !/^\s*(?:\/\/|#|\*)/.test(l)).slice(-60).join('\n')
  const pytest = relevant.match(/(?:^|\n|=+)\s*(\d+) passed(?:[,\s]+(\d+) failed)?[^\n]*$/m)
  const failedFirst = relevant.match(/(?:^|\n|=+)\s*(\d+) failed(?:[,\s]+(\d+) passed)?[^\n]*$/m)
  ...
  if (jest && (jest[1] || jest[2])) return { passed: jest[2] ? +jest[2] : 0, failed: jest[1] ? +jest[1] : 0, status: +jest[1] > 0 ? 'fail' : 'pass' }
  ...
  if (failedFirst) return { passed: failedFirst[2] ? +failedFirst[2] : null, failed: +failedFirst[1], status: 'fail' }
  if (pytest) return { passed: +pytest[1], failed: pytest[2] ? +pytest[2] : 0, status: pytest[2] && +pytest[2] ? 'fail' : 'pass' }
...
function checkObservedEdit(edit: EditSuggestion, observation: ScreenObservation, snapshot: RepoSession['snapshot']): EditSuggestion {
  ...
  const afterAnchor = anchorEdit(file, edit.after)
  if (afterAnchor !== null) return { ...edit, status: 'matched', note: 'Suggested text is visible. This is not a correctness or test verdict.' }
  ...
  return { ...edit, status: 'different', note: operatorsDiffer ? 'Boolean operator differs from the suggestion. Check the requirement before changing it.' : 'Observed code differs. It may be an equivalent solution; review semantics rather than assuming it is wrong.' }
}
export function reduceRepoEvent(previous: RepoSession, input: RepoEvent): RepoSession {
  ...
  switch (e.kind) {
    ...
    case 'observation': {
      const o = e.data.observation
      const snapshot = mergeScreenObservation(s.snapshot, o)
      ...
      // Repeated captures can have new provenance without new usable information.
      const representation = (x: typeof snapshot) => JSON.stringify({ files: x.files.map(f => [f.path, f.revision, [...observedLines(f)].map(([n, l]) => [n, l.text, l.confidence])]), paths: x.visiblePaths, terminal: x.terminal, requirements: x.requirements })
      const meaningful = representation(snapshot) !== representation(s.snapshot)
      const codeRevision = s.codeRevision + (changed ? 1 : 0)
      ...
      const testRuns = s.testRuns.map(run => {
        if (run.codeRevision !== codeRevision) return { ...run, status: 'stale' as const }
        if (run.status !== 'waiting' || !o.terminal || o.terminal === run.initialTerminal || e.at <= run.startedAt) return run
        const result = parseTestSummary(o.terminal)
        return result ? { ...run, status: result.status === 'pass' ? 'observed-pass' as const : 'observed-fail' as const, output: o.terminal, passed: result.passed, failed: result.failed } : run
      })
    ...
    case 'say': {
      const ok = e.data.stamp.sessionId === s.id && e.data.stamp.questionId === s.question?.id && e.data.stamp.codeRevision === s.codeRevision
    ...
    case 'plan': {
      ...
      if (!s.holdImplementation) for (const [i, edit] of p.edits.entries()) {
        const file = s.snapshot.files.find(f => f.path === edit.path)
        const line = anchorEdit(file, edit.before)
        if (!file || line === null) { rejected.push(`Recapture ${edit.path}: the proposed preimage is missing, ambiguous, or uncertain.`); continue }
    ...
    case 'test-start': return { ...s, testRuns: [...s.testRuns, { id: `${s.id}:${e.seq}`, command: e.data.command, codeRevision: s.codeRevision, startedAt: e.at, initialTerminal: s.snapshot.terminal, status: 'waiting', output: '', passed: null, failed: null }].slice(-30) }
  }
}
export function exportReplay(s: RepoSession): string {
  if (s.journalTruncated) throw new Error('Event history limit reached. Start a shorter session for a complete replay.')
  const result = JSON.stringify({ format: 'livetranscript-repo-replay-v1', sessionId: s.id, startedAt: s.startedAt, events: s.events }, null, 2)
  if (new TextEncoder().encode(result).byteLength > MAX_REPLAY_BYTES) throw new Error('Replay exceeds 8 MB; use a shorter session.')
  return result
}
export function importReplay(text: string): RepoSession {
  ...
  if (r.format !== 'livetranscript-repo-replay-v1') throw new Error('Unsupported replay format')
  let s = createSession(txt(r.sessionId, 100), num(r.startedAt))
  for (const raw of arr(r.events, MAX_EVENTS)) {
    const e = parseEvent(raw)
    if (e.sessionId !== s.id || e.seq !== s.seq + 1) throw new Error('Replay identity or sequence mismatch')
    s = reduceRepoEvent(s, e)
  }
  return s
}

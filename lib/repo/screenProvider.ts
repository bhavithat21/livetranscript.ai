...
For the inline annotation mode ONLY, each file entry may include optional lineRects:[{line:12,rect:{x:0.2,y:0.3,width:0.5,height:0.02}}]. Coordinates are normalized to the WHOLE supplied screenshot (top-left 0,0; bottom-right 1,1). Give the actual visible source-text rectangle for that exact numbered line, excluding its number gutter. At most 80 rows per file. Do not infer rows by uniform line height, including across wrapping/folding. Omit any clipped, wrapped or ambiguous row. Never label a file from a code suggestion or annotation as the actual IDE file. Omit lineRects entirely if you cannot locate visible lines. For each file with lineRects, also optionally supply trackingRegions:{editor:{x,y,width,height},identity:{x,y,width,height},watch:[{x,y,width,height}]}. editor is the visible scrolling CODE TEXT viewport for that pane, excluding fixed gutter, minimap, scrollbar tracks, tab bar and terminal. identity is a stationary tab/breadcrumb rectangle actually showing that file's identity, not a guessed band above the first complete row. watch is zero to three separate visible terminal/test/problem-output rectangles (exclude their scrollbars). All rectangles use whole-screenshot normalized coordinates. Do not infer a viewport from text line spacing. Omit trackingRegions if boundaries or identity are ambiguous; missing regions disables continuous tracking rather than guessing. All original exact transcription rules still apply.` : ''),

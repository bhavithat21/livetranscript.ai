import { parseLineRects } from './inline/geometry'
...
export function parseObservation(raw: unknown): Observation {
  ...
  for (const rawFile of list(root.files, 12)) {
    const item = object(rawFile, ['path', 'language', 'startLine', 'lines', 'confidence', 'endOfFile', 'lineRects'])
    ...
    files.push({ path, language, startLine, lines, confidence: probability(item.confidence), endOfFile: item.endOfFile, ...(item.lineRects === undefined ? {} : { lineRects: parseLineRects(item.lineRects, startLine, lines) }) })
  }

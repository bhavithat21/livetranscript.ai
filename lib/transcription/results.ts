...
function partsFor(text: string, words: Word[], unit: number, label: (value: unknown) => number | null, fallback?: unknown): TranscriptPart[] | undefined {
  ...
  for (let i = 0; i < words.length; i++) {
    const word = words[i], speaker = word.word_is_final === false ? null : label(word.speaker === undefined ? fallback : word.speaker)
...
export function assemblyResult(data: ResultMessage, stream: string, label: (value: unknown) => number | null): TranscriptEvent | null {
  ...
  const words = wordsOf(data.words)
  const wordLabel = (word: Word) => word.word_is_final === false ? null : label(word.speaker === undefined ? data.speaker_label : word.speaker)
  ...
  return {
    ...
    ...(data.end_of_turn === true ? { parts: partsFor(data.transcript, words, 1, label, data.speaker_label) } : {}),
  }
}
